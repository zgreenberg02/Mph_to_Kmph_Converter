...
public class Mph_to_Kmph extends PApplet {
...
public void draw()
{
  ...
  kph = PApplet.parseInt(mph*1.60934f);
}
...
public void drawRef()
{
  ...
  for (float i = 135; i <= 405; i = i + (270/17))
  {
    ...
    float c1x2 = c1x + cos(angle1) * radius * 0.9f;
    float c1y2 = cy + sin(angle1) * radius * 0.9f;
  ...
  for (float i = 135; i <= 405; i = i + (270/16))
  {
    ...
    float c2x2 = c2x + cos(angle2) * radius * 0.9f;
    float c2y2 = cy + sin(angle2) * radius * 0.9f;
  ...
  for (float i = 135; i <= 405; i = i + 60)//2*(270/17))
  {
    ...
    float c1x2 = c1x + cos(angle3) * radius * 0.8f;
    float c1y2 = cy + sin(angle3) * radius * 0.8f;
    int text1 = PApplet.parseInt(i / 1.5f - 90);
    String t1 = ""+text1;
    float c1tx = c1x + cos(angle3) * radius * .67f - textWidth(t1) + 5;
    float c1ty = cy + sin(angle3) * radius * .67f + 5;
    strokeWeight(3);
    stroke(0);
    text(t1, c1tx,c1ty);
    line(c1x1, c1y1, c1x2, c1y2);
  ...
  for (float i = 135; i <= 405; i = i + 2*(270/16))  
  {
    ...
    float c2x2 = c2x + cos(angle4) * radius * 0.8f;
    float c2y2 = cy + sin(angle4) * radius * 0.8f;
    strokeWeight(3);
    stroke(0);
    line(c2x1, c2y1, c2x2, c2y2);
    ...
    int text2 = PApplet.parseInt(i * 32 / 27 - 160);  //need to change 
    String t2 = ""+text2;
    float c2tx = c2x + cos(angle4) * radius * .67f - textWidth(t2) + 5;
    float c2ty = cy + sin(angle4) * radius * .67f + 5;
  ...
  float radm = radians(1.5f * mph + 135);
  float radk = radians(135 * kph / 160 + 135);
  float l1x = c1x + cos(radm) * radius * .9f;
  float l1y = cy + sin(radm) * radius * .9f; 
  float l2x = c2x + cos(radk) * radius * .9f;
  float l2y = cy + sin(radk) * radius * .9f; 
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Mph_to_Kmph" };
    if (passedArgs != null) {
